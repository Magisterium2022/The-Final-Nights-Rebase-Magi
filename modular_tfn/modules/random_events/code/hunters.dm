...
/mob/living/carbon/human/npc/walkby/hunter/Initialize(mapload)
	...
	var/willpower = rand(5, 8)
	storyteller_stats = create_new_stat_prefs(storyteller_stats) //For reference, the average Human has stats of 2, and skills of 0 or 1. 5 Represents the absolute peak of human ability. 76% of humans have WP 5 or below, with only 2% being above 8.
	st_set_stat(STAT_BRAWL, rand(1, 3)) 
	st_set_stat(STAT_STAMINA, rand(2, 4))
	st_set_stat(STAT_DEXTERITY, rand(2, 4))
	st_set_stat(STAT_STRENGTH, rand(2, 4))
	st_set_stat(STAT_MELEE, rand(1, 3))
	st_set_stat(STAT_PERCEPTION, rand(2, 4))
	st_set_stat(STAT_PERMANENT_WILLPOWER, willpower)
	st_set_stat(STAT_TEMPORARY_WILLPOWER, willpower)
